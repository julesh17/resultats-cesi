...
### Correction création de compte pour une base déjà installée

Si `supabase/setup.sql` avait déjà été exécuté avant cette version et que la création d'un compte affiche `permission denied for table profiles`, exécuter **une seule fois** `supabase/fix_signup.sql` dans **Supabase > SQL Editor**. Les nouvelles installations qui exécutent le `setup.sql` actuel incluent déjà ce correctif.
