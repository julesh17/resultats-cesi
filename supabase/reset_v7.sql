...
-- L'identité d'une matière issue du fichier de notes est la colonne Excel elle-même.
-- Deux libellés proches ne sont donc jamais fusionnés pendant l'import des notes.
create unique index if not exists evaluations_source_key_unique
on public.evaluations(session_id, source_key)
where source_key is not null;
create index if not exists idx_evaluations_exact_name
on public.evaluations(session_id, semester, normalized_name);
