...
export default function DettesPage() {
  ...
  async function setDebtStatus(debt: Debt, next: 'pending' | 'validated') {
    ...
    const { error } = await supabase.from('debts').update({
      status: next,
      validated_at: next === 'validated' ? now : null,
      status_manual: true,
      status_updated_at: now,
      status_updated_by: auth.user?.id || null,
    }).eq('id', debt.id);
    if (error) { setMessage(error.message); return; }
    setMessage(next === 'validated' ? 'Dette validée et enregistrée.' : 'Dette invalidée et enregistrée.');
    await loadDebts();
  }
  ...
  return <div className="space-y-6">
    ...
    {loading ? <Loading /> : <section className="card overflow-hidden"><div className="overflow-auto"><table className="min-w-full border-collapse"><thead><tr><th className="table-header">Étudiant</th><th className="table-header">UE</th><th className="table-header">Origine</th><th className="table-header">Statut</th><th className="table-header">Informations</th><th className="table-header text-right">Actions</th></tr></thead><tbody>{debts.map((d) => <tr key={d.id} className="border-t" style={{ borderColor:'var(--border)' }}><td className="table-cell font-medium whitespace-nowrap">{d.students ? displayStudent(d.students.first_name, d.students.last_name) : '—'}</td><td className="table-cell">{d.ues?.name || 'UE supprimée'}</td><td className="table-cell whitespace-nowrap">{d.origin_year_label} · S{d.origin_semester}</td><td className="table-cell min-w-[190px]"><select className="form-select py-2" value={d.status} onChange={(e) => setDebtStatus(d, e.target.value as 'pending' | 'validated')} aria-label={`Statut de la dette de ${d.students ? displayStudent(d.students.first_name, d.students.last_name) : 'cet étudiant'}`}><option value="pending">Dette non validée</option><option value="validated">Dette validée</option></select></td><td className="table-cell max-w-[320px]"><button className="text-left text-sm hover:underline" onClick={() => setEditing(d)}>{d.notes || <span className="muted">Ajouter une note…</span>}</button></td><td className="table-cell"><div className="flex justify-end gap-2">{d.status === 'pending' ? <button className="btn-success" onClick={() => setDebtStatus(d, 'validated')}><CheckCircle2 size={15} /> Valider la dette</button> : <button className="btn-secondary" onClick={() => setDebtStatus(d, 'pending')}><RotateCcw size={15} /> Invalider la dette</button>}</div></td></tr>)}{!debts.length ? <tr><td colSpan={6} className="p-10 text-center text-sm muted">Aucune dette dans cette vue.</td></tr> : null}</tbody></table></div></section>}
